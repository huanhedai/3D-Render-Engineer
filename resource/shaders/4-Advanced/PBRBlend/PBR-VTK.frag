...
void main()
{
    ...
    if(gl_FrontFacing == false)
        localNormal = -localNormal;   // 背面法线取反
    vec3 coatNormal = localNormal;   // 涂层法线初始化为基础法线

    vec3 tangentVC = tangent;
    // 容错：如果tangent是零向量，用默认方向构建TBN
    if(length(tangentVC) < 1e-5) {
        tangentVC = vec3(1.0, 0.0, 0.0); // 默认切线方向
        tangentVC = normalize(tangentVC - dot(tangentVC, localNormal) * localNormal);
    } else {
        tangentVC = normalize(tangentVC - dot(tangentVC, localNormal) * localNormal);
    }
    vec3 bitangentVC = cross(localNormal, tangentVC);
    mat3 TBN = mat3(tangentVC, bitangentVC, localNormal);
    ...
    localNormal = normalize(TBN * NormalTS);
    // localNormal = getNormalFromMap();
     localNormal = coatNormal;
    // PBR Impl
    vec4 albedoSample = texture(albedoMap, UV);
    vec3 albedo = albedoSample.rgb * diffuseColor;      // 模型带颜色 + 模型贴图 混合
    ...
    //float ao = texture(albedoMap, UV).r;
    ...
    vec2 brdf = vec2(1.0, 1.33); // BRDF 
    vec3 prefilteredSpecularCoatColor = vec3(0.0); // 涂层预过滤环境光照
    vec2 coatBrdf = vec2(1.0, 1.33); // 涂层 BRDF 
    ...
    for(int i = 0; i < MAX_SPOT_LIGHTS;i++){
        ...
        if(spotLights[i].outerLine < 90.0){
            ...
            if(coneDot >= cos(radians(spotLights[i].outerLine))
            {
                ...
                float intensity = clamp((cGamma - spotLights[i].outerLine) / (spotLights[i].innerLine - spotLights[i].outerLine),0.0, 1.0);
