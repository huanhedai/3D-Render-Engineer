...
void prepare() {
    ...
    std::string pbrFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/PBRBlend/PBR-jinglian_2.frag";
    ...
    std::vector<ShaderMacro> pbrMacros = {
        ...
        ShaderMacro("MAX_SPOT_LIGHTS", "1", ShaderTarget::FRAGMENT, "最大聚光灯光源数量"),
        ShaderMacro("CLEAR_COAT", ShaderTarget::FRAGMENT),
    };
    ...
    Shader* PBRShader = new Shader(pbrVertexPath.c_str(), pbrFragmentPath.c_str(), pbrMacros);
    ...
    directionLights.push_back(dirLight);
...
int main(int argc, char* argv[]) {
    ...
    while (App->update()) {
        ...
        renderer->render(meshes, camera, directionLights, pointLights, spotLights, ambLight);
