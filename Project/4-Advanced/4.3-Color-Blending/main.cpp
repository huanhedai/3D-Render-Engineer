...
void prepare() {
	...
	auto materialBox = new PBRMaterial();
	materialBox->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/window.png", 0);
	materialBox->setShader(BoxShader);
    materialBox->mBlend = true;
	materialBox->mDepthWrite = false;
    materialBox->mOpacity = 0.3f;

	auto meshBox = new Mesh(geometryBox, materialBox);
	 meshes.push_back(meshBox);

	auto matHeart = new PBRMaterial();
    ...
    matHeart->mAlbedoMap = new Texture(TalbedoPath, 1);
    //matHeart->mNormalMap = new Texture(TnormalPath, 1);
    //matHeart->mMetallicMap = new Texture(TmetallicPath, 2);
    //matHeart->mRoughnessMap = new Texture(TroughnessPath, 3);
    //matHeart->mAoMap = new Texture(TaoPath, 4);

    matHeart->setShader(BoxShader);
    matHeart->mBlend = true;
    matHeart->mDepthWrite = false;
    matHeart->mOpacity = 0.5f;

	auto meshHeart = new Mesh(geometryHeart, matHeart);
    meshHeart->addChild(meshBox);
