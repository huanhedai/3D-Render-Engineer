...
void prepare() {
    ...
    // ----------------A方块实体与边界
    // 1 普通方块
    auto Boxgeometry = Geometry::createBox(4.0f);
    auto materialA = new PBRMaterial();
    materialA->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/R-D.jpg", 5);
    materialA->setShader(BoxShader);
    // 模板测试
    ...
    // 面剔除
	materialA->mFaceCulling = true;     // 打开面剔除
    materialA->mFrontFace = GL_CCW; // 逆时针为正面
	materialA->mCullFace = GL_BACK; // 剔除背面

    auto meshA = new Mesh(Boxgeometry, materialA);

    // 2 创建勾边方块
    auto materialABound = new WhiteMaterial();
    materialABound->setShader(WhiteShader);
    materialABound->mDepthTest = false;         // 两个物体需要共享边界的时候，边界盒子的深度检测需要关闭，这样边界就不会遮挡应该绘制的物体
    ...
    meshes.push_back(meshABound);

    // ----------------B方块实体与边界
    // 1 普通方块
    auto materialB = new PBRMaterial();
    materialB->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/wood.png", 6);
    materialB->setShader(BoxShader);
    ...
    // 面剔除
    materialB->mFaceCulling = false;    // 关闭面剔除
    materialB->mFrontFace = GL_CW; // 顺时针为正面
    materialB->mCullFace = GL_BACK; // 剔除背面

    auto meshB = new Mesh(Boxgeometry, materialB);
    meshB->setPosition(glm::vec3(1.0f, 1.0f, 1.0f));
    meshes.push_back(meshB);

    // 2 创建勾边方块
    auto materialBBound = new WhiteMaterial();
    materialBBound->setShader(WhiteShader);
    materialBBound->mDepthTest = false;     
    // 如果关闭了深度测试（比如调用 glDisable(GL_DEPTH_TEST)），
    // OpenGL 就会跳过遮挡判断，采用「后绘制的像素直接覆盖先绘制的像素」的 “画家算法” 逻辑
