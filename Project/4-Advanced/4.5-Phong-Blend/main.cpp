...
void renderIMGUI_config() {
    ...
    if (ImGui::TreeNode("Material Settings")) {
        ...
        ImGui::Text("Opacity"); 
        ImGui::SliderFloat("Opacity", &materialConfig.Opacity, 0.0f, 1.0f);
        ImGui::TreePop();
    }
    ImGui::Separator();
...
void prepare() {
   ...
   auto PBRgeometry = Geometry::createFromOBJ("E:/IT-Furnace/OpenGl/mindray/Framework/resource/objs/pbrheart/pbrheart.obj");
    ...
    // 创建Phongmaterial并且配置参数
    std::string texturePath = std::string(TEXTURE_DIR) + "/pbrheart/Albedo.png";

    phongmaterial->mDiffuse = new Texture(texturePath, 0);
    ...
    phongmaterial->mSpecularMask = new Texture(texturePath, 1);  // 与区分开来
    
    phongmaterial->mBlend = true;
...
void applyImGuiSettings_config() {
    ...
    phongmaterial->mOpacity =   materialConfig.Opacity;
