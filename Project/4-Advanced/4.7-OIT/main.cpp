...
#include "../../../include/glframework/material/screenMaterial.h"
...
std::vector<Mesh*> meshesOffScreen{};
std::vector<Mesh*> meshesInScreen{};
...
// OIT专用FBO及附件（全局，和你的fbo变量同级）
unsigned int oitFBO;
Texture* oitColorWeightTex;  // 存储 color * weight（RGBA32F格式）
Texture* oitWeightSumTex;    // 存储 weight总和（RGBA32F格式，仅用r通道）
unsigned int oitDepthTex;    // OIT深度附件（用于透明物体自身深度测试）

// 复合着色器（用于合并OIT结果）
Shader* oitCompositeShader;
// 复合材质（全屏四边形用）
Material* oitCompositeMaterial;
Mesh* oitCompositeMesh;
...
void prepareOITFBO() {
    // 1. 创建OIT FBO
    glGenFramebuffers(1, &oitFBO);
    glBindFramebuffer(GL_FRAMEBUFFER, oitFBO);

    // 2. 创建「color×weight」纹理（RGBA32F浮点格式，高精度）
    oitColorWeightTex = new Texture(WIDTH, HEIGHT,0);
    // 绑定到FBO颜色附件0
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, oitColorWeightTex->getID(), 0);

    // 3. 创建「weight总和」纹理（同样RGBA32F，仅用r通道存储总和）
    oitWeightSumTex = new Texture(WIDTH, HEIGHT, 1);
    // 绑定到FBO颜色附件1
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, oitWeightSumTex->getID(), 0);

    // 4. 创建深度附件（透明物体自身深度测试用）
    glGenTextures(1, &oitDepthTex);
    glBindTexture(GL_TEXTURE_2D, oitDepthTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT24, WIDTH, HEIGHT, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, oitDepthTex, 0);

    // 5. 告诉FBO要绘制到两个颜色附件
    GLenum oitDrawBuffers[] = { GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1 };
    glDrawBuffers(2, oitDrawBuffers);

    glClearBufferfv(GL_COLOR, 0, glm::value_ptr(glm::vec4(0.0f))); // 清空colorWeight纹理
    glClearBufferfv(GL_COLOR, 1, glm::value_ptr(glm::vec4(0.0f))); // 清空weightSum纹理

    // 6. 检查OIT FBO完整性
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "OIT FBO 不完整！错误码：" << glCheckFramebufferStatus(GL_FRAMEBUFFER) << std::endl;
    }
    ...
    // 7. 创建复合着色器（合并OIT结果）
    std::string compositeVertPath = std::string(SHADER_DIR) + "/4-Advanced/oit/composite.vert";
    std::string compositeFragPath = std::string(SHADER_DIR) + "/4-Advanced/oit/composite.frag";
    oitCompositeShader = new Shader(compositeVertPath.c_str(), compositeFragPath.c_str());

    // 8. 创建复合材质（全屏四边形用）
    oitCompositeMaterial = new ScreenMaterial();
    oitCompositeMaterial->setShader(oitCompositeShader);
    oitCompositeMaterial->mBlend = true;

    // 给复合材质传递OIT的两个纹理附件
    ((ScreenMaterial*)oitCompositeMaterial)->mColorWeightTexture = oitColorWeightTex;
    ((ScreenMaterial*)oitCompositeMaterial)->mWeightSumTexture = oitWeightSumTex;

    // 9. 创建全屏四边形
    oitCompositeMesh = new Mesh(Geometry::createScreenPlane(), oitCompositeMaterial);

	meshesInScreen.push_back(oitCompositeMesh);
}
...
void prepare() {
    std::string pbrVertexPath = std::string(SHADER_DIR) + "/4-Advanced/oit/PBRBlend.vert";
    std::string pbrFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/oit/PBRBlend.frag";

    std::string phongVertexPath = std::string(SHADER_DIR) + "/4-Advanced/oit/PhongBlend.vert";
    std::string phongFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/oit/PhongBlend.frag";

	Shader* PBRShader = new Shader(pbrVertexPath.c_str(), pbrFragmentPath.c_str());
    Shader* PhongShader = new Shader(phongVertexPath.c_str(), phongFragmentPath.c_str());
...

    {
        ...
        pbrmaterial->mAlbedoMap = new Texture(TalbedoPath, 3);
        //pbrmaterial->mNormalMap = new Texture(TnormalPath, 4);

        pbrmaterial->setShader(PBRShader);
        pbrmaterial->mBlend = false;
        pbrmaterial->mDepthWrite = false;
        pbrmaterial->mOpacity = 0.5f;
    }
    {
        phongmaterial->mDiffuse = new Texture(std::string(TEXTURE_DIR) + "/pbrheart/Albedo.png", 3);
        phongmaterial->setShader(PhongShader);
        phongmaterial->mBlend = false;
        phongmaterial->mDepthWrite = false;
        phongmaterial->mOpacity = 0.5f;

    }
    auto meshHeart = new Mesh(geometryHeart, pbrmaterial);
	meshesOffScreen.push_back(meshHeart);
...
int main(int argc, char* argv[]) {
    if (!App->init(WIDTH, HEIGHT,"FBO TEST")) {	// 初始化Application类，创建窗体
    ...
    prepareCamera();
	prepareOITFBO();       // 准备FBO，顺序问题，必须在prepare()之前，因为prepare()中会用到FBO的颜色附件纹理
    prepare();
    initIMGUI();
    ...
    while (App->update()) {
	    ...
	    // 1. 离屏渲染
		renderer->render(meshesOffScreen, camera, dirLight, pointLights, spotLight, ambLight, oitFBO);

		// 2. 屏幕渲染
        renderer->render(meshesInScreen, camera, dirLight, pointLights, spotLight, ambLight);
