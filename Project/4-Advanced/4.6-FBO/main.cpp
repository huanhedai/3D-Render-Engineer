...
// 窗口颜色
glm::vec3 clearColor{};
unsigned int fbo = 0;   // 帧缓冲对象ID
Texture* colorAttachment = nullptr; // FBO颜色附件纹理
...
void prepareFBO() {
	...
	// 2、创建一个纹理并配置为FBO的颜色附件
	colorAttachment = new Texture(WIDTH, HEIGHT, 0);    // 创建一个空白纹理，绑定到0号纹理单元，作为颜色附件
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorAttachment->getID(), 0);   // 将纹理作为颜色附件, 最后一个参数是mipmap层级，0表示基本层级

	// 3、创建一个渲染缓冲对象并配置为FBO的深度和模板附件
	unsigned int depthStencil;      // 深度模板渲染缓冲对象ID
	glGenTextures(1, &depthStencil);
	glBindTexture(GL_TEXTURE_2D, depthStencil);     // 不需要激活，直接绑定为纹理对象，为什么不需要激活？因为我们不需要采样它，只是作为附件使用
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT, GL_TEXTURE_2D, depthStencil, 0);// 将纹理作为深度模板附件

	// 4、检查FBO是否完整
    GLenum fboStatus = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    if (fboStatus != GL_FRAMEBUFFER_COMPLETE) {
        std::cout << "FBO 不完整！错误码：" << fboStatus << std::endl;
        // 对应错误码含义（参考 OpenGL 文档）
        switch (fboStatus) {
        case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
            std::cout << "原因：某个附件的格式不被支持，或附件未正确绑定" << std::endl;
            break;
        case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
            std::cout << "原因：FBO 没有任何有效附件" << std::endl;
            break;
        case GL_FRAMEBUFFER_UNSUPPORTED:
            std::cout << "原因：附件组合的格式不被显卡支持" << std::endl;
            break;
        default:
            std::cout << "原因：其他未明确错误" << std::endl;
        }
    }
...
void prepare() {
	...
	std::string screenVertexPath = std::string(SHADER_DIR) + "/4-Advanced/screenShader/screen.vert";
	std::string screenFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/screenShader/screen.frag";

    std::string phongVertexPath = std::string(SHADER_DIR) + "/4-Advanced/PhongBlend/vertexShader.vert";
    std::string phongFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/PhongBlend/fragmentShader.frag";

    renderer = new Renderer();

	Shader* ScreenShader = new Shader(screenVertexPath.c_str(), screenFragmentPath.c_str());
	...
	auto geometryBox = Geometry::createSphere(1.0);
	auto materialBox = new PhongMaterial();
	materialBox->setShader(PhongShader);
	materialBox->mDiffuse = new Texture(std::string(TEXTURE_DIR) + "/wood.png", 0);

	auto materialPBRBox = new PBRMaterial();
	materialPBRBox->setShader(pbrShader);
	materialPBRBox->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/wood.png", 0);    

	auto meshBox = new Mesh(geometryBox, materialPBRBox);
	meshesOffScreen.push_back(meshBox);

	auto geometryScreen = Geometry::createScreenPlane();
	auto materialScreen = new ScreenMaterial();
	materialScreen->setShader(ScreenShader);
	materialScreen->mScreenTexture = colorAttachment;   // !!! 将FBO的颜色附件纹理传给ScreenMaterial材质, 用于在屏幕上显示
   std::cout<< materialScreen->mScreenTexture->getUnit()<<"ID"<<std::endl;
    auto meshScreen = new Mesh(geometryScreen, materialScreen);
    meshesInScreen.push_back(meshScreen);
    /*
	整个过程总结：
		在prepareFBO()中，我们创建并配置了一个FBO，附加了一个颜色纹理和一个深度模板渲染缓冲。
		在prepare()中，我们创建了一个PBR材质的盒子Mesh，并将其添加到离屏渲染队列meshesOffScreen中，将渲染结果渲染到绑定的fbo上。
		然后，我们创建了一个屏幕平面Mesh，并使用ScreenMaterial材质，将fbo的颜色附件纹理赋值给材质的mScreenTexture属性
                                                                    / 从fbo的颜色附件进行纹理采样。
    */
...
int main(int argc, char* argv[]) {
    ...
    prepareCamera();
	prepareFBO();       // 准备FBO，顺序问题，必须在prepare()之前，因为prepare()中会用到FBO的颜色附件纹理,必须先创建
    prepare();
    initIMGUI();
    ...
    while (App->update()) {
        ...
        //UpdateLightAndWhite();
    /*
    * 离屏渲染（PBR 球+光照）→ 写入 FBO 的 colorAttachment → 
        屏幕渲染（全屏四边形采样 colorAttachment）→ 显示光照后的结果
    ...
    */
	    // 1. 离屏渲染
		// pass01 将Box渲染到colorAttachment纹理上，新的fbo上
		renderer->render(meshesOffScreen, camera, dirLight, pointLights, spotLight, ambLight, fbo);
		// 2. 屏幕渲染
		// pass02 将colorAttachment作为纹理渲染到屏幕上
