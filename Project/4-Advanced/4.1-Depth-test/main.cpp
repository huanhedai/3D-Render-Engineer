...
void prepare() {
    ...
    std::string pbrFragmentPath = std::string(SHADER_DIR) + "/PBR-Light/PBR-jinglian_1.frag";
    ...
    auto PBRgeometry = Geometry::createFromOBJwithTangent("E:/IT-Furnace/OpenGl/mindray/Framework/resource/objs/pbrheart/pbrheart.obj");
    auto Boxgeometry = Geometry::createPlane(10.0f,10.0f);
    ...
    /*
    这里必须使用不同的纹理单元，因为要到最后才绘制，不是绑定一个纹理就绘制一次，
    如果只是用一个纹理单元，最后才绘制，那么这个纹理单元上存储的就是最后一次绑定的纹理图片。

    在这里激活和绑定纹理单元之后，需要在传入shader的时候指定 Sampler 采样的纹理单元对象 ->SetInt
    */

	pbrmaterial->setShader(new Shader(pbrVertexPath.c_str(), pbrFragmentPath.c_str()));

    std::string BoxVertexPath = std::string(SHADER_DIR) + "/4-Advanced/normalshader/vertex.glsl";
    std::string BoxFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/normalshader/fragment.glsl";

    std::string PlaneVertexPath = std::string(SHADER_DIR) + "/4-Advanced/vertexShader.vert";
    std::string PlaneFragmentPath = std::string(SHADER_DIR) + "/4-Advanced/fragmentShader_DepthTest.frag";

	Shader* BoxShader = new Shader(BoxVertexPath.c_str(), BoxFragmentPath.c_str());
	Shader* PlaneShader = new Shader(PlaneVertexPath.c_str(), PlaneFragmentPath.c_str());

	Boxmaterial->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/R-D.jpg", 5);
    materialC->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/container2.png", 6);
	materialD->mAlbedoMap = new Texture(std::string(TEXTURE_DIR) + "/rusted_iron/albedo.png", 7);

    Boxmaterial->setShader(BoxShader);
    materialC->setShader(BoxShader);
    materialD->setShader(BoxShader);

    // 关闭深度写入
    //materialC->mDepthWrite = false;

    // 开启PolygonOffsetType
    materialC->mPolygonOffset = true;
    materialC->mPolygonOffsetType = GL_POLYGON_OFFSET_FILL;
    materialC->mFactor = 1.0f;

    // 3 生成mesh
    auto PBRmesh = new Mesh(PBRgeometry, pbrmaterial);
    auto BoxMesh = new Mesh(Boxgeometry, Boxmaterial);
    auto Mesh3 = new Mesh(Boxgeometry, materialC);
	auto Mesh4 = new Mesh(Boxgeometry, materialD);
    ...
    BoxMesh->setAngleY(0.0f);
    BoxMesh->setAngleX(-45.0f);
    BoxMesh->setScale(glm::vec3(1.0f));

    Mesh3->setPosition(glm::vec3(0.0f, 1.0f, -6.0f));
    Mesh3->setAngleY(0.0f);
    Mesh3->setAngleX(-45.0f);
    Mesh3->setScale(glm::vec3(1.0f));

    Mesh4->setPosition(glm::vec3(5.0f, 3.0f, -8.0f));
    Mesh4->setAngleY(0.0f);
    Mesh4->setAngleX(0.0f);
    Mesh4->setScale(glm::vec3(1.0f));
