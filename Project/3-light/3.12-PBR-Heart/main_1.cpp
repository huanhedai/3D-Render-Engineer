...
#include "../../../include/glframework/light/lightManager.h"
...
struct PointLightStruct {
    glm::vec3 position = glm::vec3(0.0f,3.0f,0.0f);     // 光源位置
...
void renderIMGUI() {
    ...
    if (ImGui::TreeNode("Directional Light Settings")) {
        ...
        ImGui::SliderFloat("Dir Light Intensity", &directionLightConfig.Intensity, 1.0f, 30.0f);
...
void prepare() {
    ...
    std::string phongFragmentPath = std::string(SHADER_DIR) + "/PBR-Light/Blinn-phong-dot-direct.frag";
    ...
    //auto PBRgeometry = Geometry::createSphere(5.0f);
    ...
    std::string texturePath = std::string(TEXTURE_DIR) + "/rusted_iron/albedo.png";
...
int main(int argc, char* argv[]) {
    if (!App->init(DEFAULT_WINDOW_WIDTH, DEFAULT_WINDOW_HEIGHT,"PBR")) {	// 初始化Application类，创建窗体
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Application init failed!");
        return -1;
    }
    // 相机配置
    ...
    auto cameraControl = std::make_unique<TrackBallCameraControl>();
    //auto cameraControl = std::make_unique<GameCameraControl>();
    ...
    // 初始化ImGui
    ImGuiManager::init(App->getWindow(), App->getGLContext());

    // 3. 加载资源和初始化场景
    LightManager::getInstance().addDefaultLights(); // 添加默认灯光
    ...
    while (App->update()) {
        ...
        renderer->render(meshes, camera.get(), LightManager::getInstance().getDirectionalLight(), LightManager::getInstance().getPointLights(), spotLight, ambLight);
