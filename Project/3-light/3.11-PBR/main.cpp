...
void prepare() {
    ...
    std::string pbrFragmentPath = std::string(SHADER_DIR) + "/PBR-Light/pbr-no.frag";

    std::string phongVertexPath = std::string(SHADER_DIR) + "/PBR-Light/PBR.vert";
    std::string phongFragmentPath = std::string(SHADER_DIR) + "/PBR-Light/Phong.frag";
    ...
    renderer = new Renderer(phongVertexPath, phongFragmentPath, "", "", pbrVertexPath, pbrFragmentPath);

    // 1 创建geometry
    auto PBRgeometry = Geometry::createSphere(1.0f);
    ...
    PBRmesh->setPosition(glm::vec3(-1.5f, 0.0f, 0.0f));
    PhongMesh->setPosition(glm::vec3(1.5f, 0.0f, 0.0f));
    ...
    meshWhite->setPosition(glm::vec3(0.0f, 0.0f, 3.0f));
	...
	dirLight->setColor(glm::vec3(1.0f, 1.0f, 1.0f));    // 全都设为0，关闭平行光
    dirLight->setDirection(glm::vec3(-1.0f));

    // 点光源
    auto pointLight1 = new PointLight();
    pointLight1->setPosition(glm::vec3(0.0f, 0.0f, 0.0f));
    pointLight1->setColor(glm::vec3(0.0f));
    pointLight1->setSpecularIntensity(10.0f);
    ...
    pointLight2->setPosition(glm::vec3(0.0f, 0.0f, 0.0f));
    pointLight2->setColor(glm::vec3(0.0f));
    pointLight2->setSpecularIntensity(10.0f);
